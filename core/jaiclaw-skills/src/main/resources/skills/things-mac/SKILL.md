...
- If DB reads fail: grant **Full Disk Access** to the calling app (Terminal for manual runs; the gateway app for automated runs).
...
- Complete/cancel: `things update --id <UUID> --auth-token <TOKEN> --completed` / `--canceled`
